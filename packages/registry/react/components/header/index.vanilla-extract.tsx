...
import { byKey, header, headerSolid, headerTransparent, headerBlur, header__brand, header__logo, header__title, header__trigger, header__nav, header__item, header__actions, headerDesktopOnly, headerMobileOnly, headerGroupInline, headerGroupDrawer, headerGroupLabel, headerGroupItems, header__menu, headerMenuInline, headerMenuTrigger, header__chevron, headerMenuContentPortal, header__backdrop, header__drawer, headerDrawerHead, headerDrawerNav, headerMenuDrawer, header__group, headerMenuTriggerLabel, headerMenuContent } from "./styles.css";
...
export const Header = React.forwardRef<HTMLElement, HeaderProps>(function Header(
  ...
  return (
    <HeaderContext.Provider value={ctx}>
      <header
        ref={setRefs}
        className={cn(header, byKey[`header--${variant}`], className)}
...
>(function HeaderBrand({ className, ...props }, ref) {
  return <div ref={ref} className={cn(header__brand, className)} {...props} />;
});
...
>(function HeaderLogo({ className, ...props }, ref) {
  return <span ref={ref} className={cn(header__logo, className)} {...props} />;
});
...
>(function HeaderTitle({ className, ...props }, ref) {
  return <span ref={ref} className={cn(header__title, className)} {...props} />;
});
...
>(function HeaderTrigger({ className, onClick, children, ...props }, ref) {
  ...
  return (
    <button
      ref={setRefs}
      type="button"
      className={cn(header__trigger, className)}
    ...
    return (
      <NavMatchContext.Provider value={navMatch}>
        <NavLocationContext.Provider value="inline">
          <nav ref={ref} className={cn(header__nav, className)} {...props}>
            {children}
          </nav>
        ...
        <aside
          ...
          aria-label="메뉴"
        >
          <div className={headerDrawerHead}>
            <HeaderTrigger />
          </div>
          <NavLocationContext.Provider value="drawer">
            <nav className={headerDrawerNav}>{children}</nav>
          </NavLocationContext.Provider>
        </aside>
...
>(function HeaderItem({ className, active, onClick, href, ...props }, ref) {
  ...
  return (
    <a
      ref={ref}
      href={href}
      className={cn(header__item, className)}
...
>(function HeaderActions({ className, ...props }, ref) {
  return (
    <div ref={ref} className={cn(header__actions, className)} {...props} />
  );
});
...
>(function HeaderDesktopOnly({ className, ...props }, ref) {
  return (
    <div ref={ref} className={cn(headerDesktopOnly, className)} {...props} />
  );
});
...
>(function HeaderMobileOnly({ className, ...props }, ref) {
  return (
    <div ref={ref} className={cn(headerMobileOnly, className)} {...props} />
  );
});
  ...
  function HeaderNavGroup({ className, label, children, ...props }, ref) {
    ...
    if (location === "inline") {
      return (
        <div
          ref={ref}
          className={cn(header__group, headerGroupInline, className)}
          {...props}
        >
          {children}
        </div>
    ...
    return (
      <div
        ref={ref}
        className={cn(header__group, headerGroupDrawer, className)}
        role="group"
        aria-label={typeof label === "string" ? label : undefined}
        {...props}
      >
        {label != null && (
          <div className={headerGroupLabel}>{label}</div>
        )}
        <div className={headerGroupItems}>{children}</div>
      </div>
    );
  },
);
...
>(function HeaderMenuTrigger({ className, children, onClick, ...props }, ref) {
  ...
  return (
    <button
      ...
      data-open={open ? "" : undefined}
      className={cn(headerMenuTrigger, className)}
      onClick={(e) => {
        setOpen(!open);
        onClick?.(e);
      }}
      {...props}
    >
      <span className={headerMenuTriggerLabel}>{children}</span>
      <ChevronDownIcon />
    </button>
  );
});
...
>(function HeaderMenuContent({ className, children, style, ...props }, ref) {
  ...
  if (location === "drawer") {
    return (
      <div
        ...
        data-open={open ? "" : undefined}
        hidden={!open}
        className={cn(headerMenuContent, className)}
        style={style}
        {...props}
      >
        {children}
  ...
  return createPortal(
    <div
      ...
      aria-labelledby={triggerId}
      data-open=""
      className={cn(headerMenuContent, headerMenuContentPortal, className)}
      style={{
        position: "absolute",
        top: pos.top,
        left: pos.left,
...
function ChevronDownIcon() {
  return (
    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden className={header__chevron}>
