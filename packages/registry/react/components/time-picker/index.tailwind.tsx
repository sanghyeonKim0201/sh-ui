...
/* ───────── Tailwind 유틸리티 클래스 (styles.css 미러링) ───────── */
...
const fieldClasses =
  "inline-flex items-center gap-[var(--space-1)] tabular-nums text-[length:var(--text-lg)]";

const segmentClasses =
  "min-w-[2ch] px-[var(--space-2)] py-[var(--space-1)] rounded-[var(--radius-sm)] text-center cursor-default select-none text-foreground focus:outline-none focus:bg-primary focus:text-primary-foreground focus-visible:outline-none focus-visible:bg-primary focus-visible:text-primary-foreground";

const separatorClasses = "text-foreground-muted";
...
export function TimePickerField() {
  ...
  return (
    <div className={fieldClasses} role="group" aria-label={messages.hours + " " + messages.minutes}>
      <Segment
        kind="hours"
      ...
      />
      <span className={separatorClasses} aria-hidden>:</span>
      <Segment
        kind="minutes"
...
export const TimePickerFooter = React.forwardRef<HTMLDivElement, TimePickerFooterProps>(
  function TimePickerFooter({ className, ...props }, ref) {
    return <div ref={ref} className={cn(footerClasses, className)} {...props} />;
  },
);
