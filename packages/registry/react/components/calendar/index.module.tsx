...
export function Calendar(props: CalendarProps) {
  ...
  return (
    <div
      className={cn(styles.calendar, numberOfMonths > 1 && styles["calendar--multi"], className)}
      aria-label={ariaLabel}
    >
      {children
        ? (
...
export const CalendarHeader = React.forwardRef<HTMLDivElement, CalendarHeaderProps>(
  function CalendarHeader({ className, ...props }, ref) {
    return <div ref={ref} className={cn(styles.calendar__header, className)} {...props} />;
  },
);

function CalendarNavPlaceholder() {
  return <span className={cn(styles.calendar__nav, styles["calendar__nav--placeholder"])} aria-hidden />;
}
...
export const CalendarGrid = React.forwardRef<HTMLDivElement, CalendarGridProps>(
  function CalendarGrid({ className, ...rest }, ref) {
    ...
    return (
      <div ref={ref} className={cn(styles["calendar__grid-wrap"], className)} {...rest}>
        <div className={styles.calendar__weekdays} role="row">
        ...
        >
          {cells.map(({ date, current }, i) => {
            ...
            if (hidden) {
              return <span key={i} className={cn(styles.calendar__cell, styles["calendar__cell--hidden"])} aria-hidden />;
            }
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isStart && styles["calendar__cell--range-start"],
                  isEnd && styles["calendar__cell--range-end"],
                )}
              >
                <button
                  type="button"
                  className={cn(
                    ...
                    !current && styles["calendar__day--outside"],
